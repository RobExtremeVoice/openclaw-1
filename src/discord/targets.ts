...
export async function resolveDiscordTarget(
  ...
  const directParse = parseDiscordTarget(trimmed, { defaultKind: "channel" });
  if (directParse && !isLikelyUsername(trimmed)) {
    return directParse;
  }
  ...
  return parseDiscordTarget(trimmed, { defaultKind: "channel" });
}
