...
import { runClaudeCliPrompt } from "./claude-cli.js";
import {
  ...
  return enqueueCommandInLane(sessionLane, () =>
    enqueueGlobal(async () => {
      ...
      const shouldUseClaudeCli = () =>
        provider === "anthropic" && lastProfileId === CLAUDE_CLI_PROFILE_ID;
      ...
      while (true) {
        ...
        try {
          ...
          const promptTools = shouldUseClaudeCli() ? [] : tools;
          const machineName = await getMachineDisplayName();
          const runtimeInfo = {
            host: machineName,
            os: `${os.type()} ${os.release()}`,
          ...
          if (shouldUseClaudeCli()) {
            log.warn(
              "routing anthropic request through claude cli (no tool support)",
              {
                profileId: lastProfileId,
                model: modelId,
              },
            );
            const cliResult = await runClaudeCliPrompt({
